...
IMPLEMENTATION MODULE CARD64; (* 32-bit LONGINT version *)
...
BEGIN
  IF n.highBits = m.highBits THEN
    ...
    (* only lowBits count *)
    RETURN gt32(n.lowBits, m.lowBits)
    
  ELSE (* n.highBits # m.highBits *)
    
    (* only highBits count *)
    RETURN gt32(n.highBits, m.highBits)
  END (* IF *)
END gt;
...
BEGIN
  IF n.highBits = m.highBits THEN
    ...
    (* only lowBits count *)
    RETURN lt32(n.lowBits, m.lowBits)
    
  ELSE (* n.highBits # m.highBits *)
    
    (* only highBits count *)
    RETURN lt32(n.highBits, m.highBits)
  END (* IF *)
END lt;
...
(* TO DO : adapt to signed semantics of component values *)
...
(* --------------------------------------------------------------------------
 * Private function:  gt32(n, m)
 * --------------------------------------------------------------------------
 * Interprets n and m as unsigned values, returns TRUE if n > m, else FALSE.
 * ----------------------------------------------------------------------- *)

PROCEDURE gt32 ( n, m : LONGINT ) : BOOLEAN;
...
(* --------------------------------------------------------------------------
 * Private function:  lt32(n, m)
 * --------------------------------------------------------------------------
 * Interprets n and m as unsigned values, returns TRUE if n < m, else FALSE.
 * ----------------------------------------------------------------------- *)

PROCEDURE lt32 ( n, m : LONGINT ) : BOOLEAN;
