...
IMPLEMENTATION MODULE Card64Math; (* 32-bit LONGINT version *)

(* CARD64 Math Library *)


IMPORT LongIntBitOps;
...
BEGIN
  ...
  index := LongIntBitOps.shr((log2n.lowBits + 1) * 1233, 12);
...
BEGIN
  ...
  (* remainder *)
  FOR index := 1 TO Bitwidth/2-2 DO
  ...
  INC(index);
  powerOf2[index].highBits := 0;
  powerOf2[index].lowBits := MIN(LONGINT);
  ...
  (* remainder *)
  FOR index := Bitwidth/2+1 TO Bitwidth-2 DO
  ...
  INC(index);
  powerOf2[index].lowBits := 0;
  powerOf2[index].highBits := MIN(LONGINT)
