...
IMPLEMENTATION MODULE Hash; (* requires 64-bit CARDINAL *)
...
BEGIN
  (* Clear all bits including and above bit 31 in hash value *)
...
BEGIN (* Hash *)
  ...
  IF TSIZE(CARDINAL) # CardBitwidth THEN
    Console.WriteChars("Library Hash requires 64-bit CARDINALs.");
    Console.WriteLn;
    HALT
  END (* IF *)
END Hash.